...
namespace es::thread
{
    ...
    std::shared_ptr<ThreadPool::Task> ThreadPool::push(std::function<void(std::shared_ptr<void>)> c_fn,
                                                       std::shared_ptr<void> data)
    {
        auto task = std::make_shared<ThreadPool::Task>(c_fn, data);
    ...
    void ThreadPool::pop(std::shared_ptr<ThreadPool::Task> work)
    {
        if (work)
        {
            {
    ...
    {
        std::shared_ptr<ThreadPool::Task> local_work{};
            ...
            // Wait for more work, or immediately continue if there is still work to do.
            {
                ...
                if (_tasks.size() == 0)
                {
                    _tasks_cv.wait(lock, [this]()
                                   { return _worker_stop || _tasks.size() > 0; });
                }
    ...
    ThreadPool::Task::Task(std::function<void(std::shared_ptr<void>)> fn, std::shared_ptr<void> dt)
        : _mutex(), _is_complete(), _is_dead(false), _callback(fn), _data(dt)
    {
    }

    void ThreadPool::Task::await_completion()
    {
        if (!_is_dead)
        {
            std::unique_lock<std::mutex> lock(_mutex);
            _is_complete.wait(lock, [this]()
                              { return this->_is_dead.load(); });
        }
    }
}
